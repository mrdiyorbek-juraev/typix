...
function $convertMentionElement(
  ...
  const mentionTrigger =
    domNode.getAttribute("data-typix-mention-trigger") || "@";
...
export class MentionNode extends TextNode {
  ...
  constructor(
    ...
    const includeTrigger =
      globalMentionNodeOptions.includeTrigger ?? DEFAULT_INCLUDE_TRIGGER;
    const displayText =
      text ??
      (includeTrigger ? `${mentionTrigger}${mentionName}` : mentionName);
...
export function $createMentionNode(
  params: CreateMentionNodeParams
): MentionNode {
