...
export const HeadingExtension = withTypixMeta(
  defineExtension({
    name: "@typix/heading",
  ...
  }),
  {
    commands: () => ({
      toggleHeading:
        (attrs: { level: 1 | 2 | 3 | 4 | 5 | 6 }) => (editor: LexicalEditor) =>
          editor.dispatchCommand(TYPIX_TOGGLE_HEADING, attrs),
      setHeading:
        (attrs: { level: 1 | 2 | 3 | 4 | 5 | 6 }) => (editor: LexicalEditor) =>
          editor.dispatchCommand(TYPIX_SET_HEADING, attrs),
    }),
    shortcuts: [
      {
        key: "1",
