...
export const CodeExtension = withTypixMeta(
  defineExtension({
    name: "@typix/code",
  ...
  {
    commands: () => ({
      toggleInlineCode: () => (editor: LexicalEditor) =>
        editor.dispatchCommand(TYPIX_TOGGLE_INLINE_CODE, undefined),
      toggleCodeBlock:
        (attrs?: { language?: string }) => (editor: LexicalEditor) =>
          editor.dispatchCommand(TYPIX_TOGGLE_CODE_BLOCK, attrs),
    }),
    shortcuts: [
