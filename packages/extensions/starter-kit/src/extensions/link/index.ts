            ...
            return effect(() => {
                ...
                return editor.registerCommand(
                    ...
                    (payload) => {
                        ...
                        const { url, ...attrs } = payload as { url: string; target?: string | null }
                        $toggleLink(url, { target: attrs.target ?? currentTarget ?? undefined })
                        return true
                    },
                    COMMAND_PRIORITY_EDITOR,
                )
    ...
    return defineTypixExtension<LinkConfig>({
        ...
        commands: {
            setLink: (resolvedConfig) => (ctx, attrs) => {
                ...
                if (!url) {
                    console.warn('[Typix] setLink requires a url attr, e.g. { url: "https://example.com" }')
                    return false
                }
                const target = (attrs?.target as string | null) ?? resolvedConfig.defaultTarget
        ...
        shortcuts: [
            { key: 'k', modifiers: ['mod'], command: 'setLink' },
        ],
    })
}
