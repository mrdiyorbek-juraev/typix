 ...
 * ```
 */
export const UnderlineExtension = (userConfig: Partial<UnderlineConfig> = {}) => {
    const resolvedConfig: UnderlineConfig = { ...userConfig }
    const lexicalExt = defineExtension({
        name: '@typix/underline',
        config: safeCast<UnderlineConfig>(resolvedConfig),
        mergeConfig(a: UnderlineConfig, b: Partial<UnderlineConfig>): UnderlineConfig {
            return { ...a, ...b }
        },
    ...
    return defineTypixExtension<UnderlineConfig>({
        ...
        shortcuts: [
            { key: 'u', modifiers: ['mod'], command: 'toggleUnderline' },
        ],
    })
}
