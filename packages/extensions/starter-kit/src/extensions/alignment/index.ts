...
export type AlignmentValue = 'left' | 'center' | 'right' | 'justify' | 'start' | 'end'
 ...
 * ```
 */
export const AlignmentExtension = (userConfig: Partial<AlignmentConfig> = {}) => {
    ...
    const lexicalExt = defineExtension({
        ...
        mergeConfig(a: AlignmentConfig, b: Partial<AlignmentConfig>): AlignmentConfig {
            return { ...a, ...b }
        },
    ...
    return defineTypixExtension<AlignmentConfig>({
        ...
        commands: {
            ...
            setAlignment: (resolvedConfig) => (ctx, attrs) => {
                ...
                const alignment = (attrs as any)?.alignment as AlignmentValue | undefined
                if (!alignment) {
                    console.warn('[Typix] setAlignment requires an alignment attr, e.g. { alignment: "center" }')
                    return false
                }
                ...
                ctx.editor.dispatchCommand(FORMAT_ELEMENT_COMMAND, alignment as ElementFormatType)
                return true
            },
