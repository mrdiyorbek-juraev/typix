            ...
            return effect(() => {
                ...
                return editor.registerCommand(
                    ...
                    (files) => {
                        ;(async () => {
                            try {
                                const currentAcceptedTypes = acceptedTypes?.value ?? DEFAULT_ACCEPTED_TYPES
                                const currentMaxFileSize = maxFileSize?.value ?? DEFAULT_MAX_FILE_SIZE
                                ...
                                for (const file of files) {
                                    const validation = validateFile(file, currentAcceptedTypes, currentMaxFileSize)
                                    if (validation.valid) {
                                        validFiles.push(file)
                                    } else {
                                        log('Validation failed:', file.name, validation.reason)
                                        onValidationError?.value?.(file, validation.reason ?? 'Unknown error')
                                    }
                                }
                                ...
                                const filesResult = await mediaFileReader(validFiles, currentAcceptedTypes)
