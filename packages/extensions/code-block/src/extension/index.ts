...
export const CodeBlockExtension = withTypixMeta(
  defineExtension({
  ...
  }),
  {
    commands: () => ({
      insertCodeBlock:
        (attrs?: { language?: string }) => (editor: LexicalEditor) =>
          editor.dispatchCommand(TYPIX_INSERT_CODE_BLOCK, attrs),
      setCodeLanguage:
        (attrs: { nodeKey: string; language: string }) =>
        (editor: LexicalEditor) =>
          editor.dispatchCommand(TYPIX_SET_CODE_LANGUAGE, attrs),
      copyCode: (attrs: { nodeKey: string }) => (editor: LexicalEditor) =>
        editor.dispatchCommand(TYPIX_COPY_CODE, attrs),
      deleteCodeBlock:
        (attrs: { nodeKey: string }) => (editor: LexicalEditor) =>
          editor.dispatchCommand(TYPIX_DELETE_CODE_BLOCK, attrs),
    }),
    shortcuts: [
      {
        key: "`",
