...
export const ImageExtension = withTypixMeta(
  defineExtension({
    name: "@typix/image",
  ...
  {
    commands: () => ({
      insertImage: (attrs: InsertImagePayload) => (editor: LexicalEditor) =>
        editor.dispatchCommand(INSERT_IMAGE_COMMAND, attrs),
      setImageAlignment:
        (attrs: { nodeKey: string; alignment: ImageAlignment }) =>
        (editor: LexicalEditor) =>
          editor.dispatchCommand(TYPIX_SET_IMAGE_ALIGNMENT, attrs),
      toggleImageCaption:
        (attrs: { nodeKey: string }) => (editor: LexicalEditor) =>
          editor.dispatchCommand(TYPIX_TOGGLE_IMAGE_CAPTION, attrs),
      deleteImage: (attrs: { nodeKey: string }) => (editor: LexicalEditor) =>
        editor.dispatchCommand(TYPIX_DELETE_IMAGE, attrs),
      duplicateImage: (attrs: { nodeKey: string }) => (editor: LexicalEditor) =>
        editor.dispatchCommand(TYPIX_DUPLICATE_IMAGE, attrs),
    }),
  }
);
