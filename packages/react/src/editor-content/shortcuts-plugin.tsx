...
import { useCurrentTypixEditor } from "../hooks/use-current-typix-editor";
...
/**
 * Binds all keyboard shortcuts declared on the editor's extensions to
 * the Lexical command pipeline. Rendered as a default plugin by
 * EditorContent; can be opted out via the `plugins` prop.
 */
export function ShortcutsPlugin() {
  const [lexicalEditor] = useLexicalComposerContext();
  const { editor: typixEditor } = useCurrentTypixEditor();

  useEffect(() => {
    if (!typixEditor) return;
