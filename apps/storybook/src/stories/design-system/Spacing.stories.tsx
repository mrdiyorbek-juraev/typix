...
function SpacingStory() {
  return (
    <div style={{
      ...
      {/* Table header */}
      <div style={{
        display: "grid", gridTemplateColumns: "120px 72px 1fr",
        paddingBottom: 8, marginBottom: 2,
        borderBottom: "1px solid #e4e4e7",
      }}>
        <span style={{ fontSize: 11, fontWeight: 600, color: "#71717a", letterSpacing: "0.04em", textTransform: "uppercase" }}>Name</span>
        <span style={{ fontSize: 11, fontWeight: 600, color: "#71717a", letterSpacing: "0.04em", textTransform: "uppercase" }}>Pixels</span>
      </div>
      ...
      {UNITS.map(({ name, px }) => (
        <div key={name} style={{
          ...
          <span style={{ fontSize: 12, color: "#18181b", fontFamily: "monospace" }}>{name}</span>
          <span style={{ fontSize: 12, color: "#71717a" }}>{px}px</span>
          {px > 0 ? (
            <div style={{
              height: 8, borderRadius: 2, background: "#006fee",
