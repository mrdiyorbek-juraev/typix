...
export const Default: StoryObj = {
  render: () => (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <label htmlFor="cb-default" style={{ fontSize: 13, color: "var(--foreground)", cursor: "pointer" }}>
  ...
  render: () => (
    <div style={{ padding: 32, display: "flex", flexDirection: "column", gap: 14 }}>
      {[
        ...
        { label: "Disabled + checked", props: { disabled: true, defaultChecked: true as const } },
      ].map(({ label, props }) => (
...
export const CheckboxGroup: StoryObj = {
  render: () => {
    ...
    return (
      <div style={{ padding: 24, maxWidth: 280, border: "1px solid var(--border)", borderRadius: 8, background: "var(--card)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {items.map(item => (
            <div key={item} style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              />
              <label htmlFor={`cb-${item}`} style={{ fontSize: 13, color: "var(--foreground)", cursor: "pointer" }}>
                {item}
              </label>
            </div>
          ))}
