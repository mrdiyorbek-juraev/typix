    ...
    <Card style={{ width: 360 }}>
      ...
      <CardContent>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <label style={{ fontSize: 12, fontWeight: 500, color: "var(--foreground)" }}>Name</label>
          <input
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {[
            ...
            <div key={title} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
              ...
              <div>
                <p style={{ margin: 0, fontSize: 13, color: "var(--foreground)" }}>{title}</p>
                <p style={{ margin: 0, fontSize: 11, color: "var(--muted-foreground)", marginTop: 2 }}>{time}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <CardContent>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {[
            ...
            { label: "Marketing emails",    desc: "News and promotions",        on: false },
            { label: "Security alerts",     desc: "Login and access alerts",    on: true  },
          ].map(({ label, desc, on }) => (
