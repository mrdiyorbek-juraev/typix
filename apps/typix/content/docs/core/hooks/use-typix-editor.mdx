...
## Difference from useEditor

| Hook | Returns | Use case |
| --- | --- | --- |
| `useTypixEditor` | `TypixEditor` | High-level fluent API (formatting, content export, etc.) |
| `useEditor` | `{ editor: LexicalEditor, isEmpty: boolean }` | Low-level Lexical access + empty state |

Use `useTypixEditor` for most cases. Use `useEditor` when you need the raw Lexical editor instance or the `isEmpty` state.
