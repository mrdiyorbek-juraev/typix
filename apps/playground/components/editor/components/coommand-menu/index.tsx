...
export function CommandMenu({ commands }: CommandMenuProps) {
    return (
        <EditorCommand className="w-72 rounded-xl border border-border/80 bg-popover/95 backdrop-blur-sm shadow-2xl ring-1 ring-black/5 overflow-hidden" items={commands} trigger="/">
            <EditorCommandEmpty className="px-4 py-3 text-muted-foreground text-sm">
                No commands found
            </EditorCommandEmpty>
            <EditorCommandList className="max-h-80 overflow-y-auto p-1">
                {commands.map((item) => (
                    <EditorCommandItem
                        key={item.title}
